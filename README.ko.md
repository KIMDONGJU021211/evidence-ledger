...
- 걸음 기록엔 도구가 돌았고 `ok` 라고 찍혀 있는데, 모델이 그 페이지를 **읽은 건지**
  검색 결과 목록에서 **제목만 본 건지** 구별이 안 된다.
- 답이 숫자를 하나 댔는데 — 가격이든 퍼센트든 건수든 — **어느 도구 결과에도 없는
  숫자**다. 어느 것부터 확인해야 할지 알고 싶다.
- 근거 대조나 인용 검사가 **100%** 라고 하는데 믿기지 않는다.
- 읽기 전용이라고 선언한 판이 **뭔가를 썼는데** 아무도 못 잡았다.
- 에이전트가 링크를 **열긴 열었고** 거기서 한 걸음을 쓰고는, 결국 **목록에서 본 것으로**
  답했다.
- 정찰기(서브에이전트)가 읽었기 때문에 부모 전사에는 그 흔적이 없고, 근거 검사가
  **조용히 통과**한다.
- 가져온 적 없는 문서를 요약했는데, 그 요약이 그럴듯해서 일주일 동안 아무도 몰랐다.
- 그 판에서 **실제로 읽힌 출처가 무엇인지** 알고 싶다. 어떤 도구가 호출됐는지 말고.
- 판이 **실패**했는데 호출한 쪽은 「실패」만 받았다 — 이미 만들어 둔 파일이 멀쩡했는데도.
  `Ledger.surviving_writes()` 가 그 판이 **남긴 것**을 돌려준다: 쓴 것에서 나중에 지운 것을 뺀 목록.

이 판들은 걸음 기록에서 정상 판과 구별되지 않습니다. 도구 결과마다 한 줄을 남기면
구별됩니다. 이 라이브러리가 하는 일이 그것뿐입니다.

어떤 프레임워크를 쓰는지는 알지도, 상관하지도 않습니다. 도구 결과 하나당 dict 하나와,
각 도구가 무엇을 하는지 적은 매니페스트만 있으면 됩니다.
...
### `numbers_in_body()` — 짝이 되는 집합

필드만 내고 본문을 안 걷은 채로 뒀더니 그 실명이 그대로 남았다. 실제 산출물 셋을
필드만으로 채점했더니 **100% 근거 없음**이 나왔고, 그 값들은 전부 살아 있는
페이지에서 손으로 찾았다.

```python
result = {"text": "432쪽 · ISBN 9791163034735", "applicants": 5}

numbers_in(result)        # {'5'}                     도구가 그렇게 말했다
numbers_in_body(result)   # {'432', '9791163034735'}  도구가 준 글자에 있었다
```

**섞지 않는다.** 합치면 필드 대조가 뜻을 잃고, 안 걷으면 대조가 아예 못 돈다.
둘 중 **어디에도** 없는 숫자는 도구가 간 적 없는 곳에서 온 것이다 — 서비스 안내
페이지와 엉뚱한 책을 열어 놓고 책 3권의 쪽수를 보고한 판에서, 이 검사가 6개 중
6개를 잡았다.
